...
- 精英化规划：按干员等级、阶段和真实仓库独立计算 E0→E1 / E1→E2
- 模组规划：支持同一干员的多个模组与 Lv.0→Lv.1→Lv.2→Lv.3，校验精二、等级和材料门槛
- `M2→M3 > M1→M2 > M0→M1` 单阶段独立规划
- 仅展示跨度至少两级的连续专精，并支持严格完全反向排序
- 多层递归、共享原料、整数批次、多产物和循环防护
- 无限材料的正式状态、合成链传播及结果依赖标记
- 首页筛选、干员档案、仓库页、材料用途/描述、直接可合成数量及三类规划详情
- 跟随系统、深色和浅色三态主题
...
测试覆盖需求中的 18 个 Case，并额外覆盖精英化、单/多模组、等级门槛、直接可合成数量、多产物整数批次、循环配方与精二/Rank 7 前置条件。
